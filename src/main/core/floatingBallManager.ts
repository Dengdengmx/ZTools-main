...
class FloatingBallManager {
  ...
  private async loadConfig(): Promise<void> {
    try {
      ...
      // 🚨 魔改 1：无视本地设置，强制开启引力坞悬浮球！
      this.enabled = true 
      
      // 🚨 魔改 2：把默认的 'Z' 换成咱们 Mtools 的极客标识 (支持 Emoji 或短字母)
      this.letter = '🧬' 
      
      // 🚨 魔改 3：清空默认双击指令，让它原生地作为唤醒主窗口的开关
      this.doubleClickCommand = ''
      ...
      console.log('[GravityDock] Mtools 引力坞已强制上线, enabled:', this.enabled)
    } catch (error) {
      console.error('[GravityDock] 加载引力坞配置失败:', error)
    }
  }
