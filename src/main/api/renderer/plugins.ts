...
export class PluginsAPI {
  ...
  private async fetchPluginMarket(): Promise<PluginMarketResult> {
    ...
    try {
      ...
      const defaultBaseUrl =
        'https://github.com/MToolsCenter/MTools-plugins/releases/latest/download'
  ...
  private async getRemotePluginReadme(
    ...
    try {
      const baseUrl = `https://raw.githubusercontent.com/MToolsCenter/MTools-plugins/main/plugins/${pluginName}`
  ...
  private async installPluginFromNpm(packageName: string, useChinaMirror = false): Promise<any> {
    try {
      ...
      try {
        ...
        // 清理临时文件
        await fs.rm(tempDir, { recursive: true, force: true })
        return { success: false, error: '这不是一个有效的 MTools 插件包（缺少 plugin.json）' }
      }
