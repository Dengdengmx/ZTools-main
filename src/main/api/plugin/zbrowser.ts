...
export class ZBrowserAPI {
  ...
  private registerHandlers(): void {
    ...
    registerPluginApiServices({
       ...
       * MTools 改为 ipcInvoke 异步（因为 session.setProxy 是 Promise）。
       ...
       * 异步 API：清除缓存
       *
       * ⚠️ 破坏性变更：同上，uTools 为同步，MTools 改为异步。
       */
       ...
       * MTools 暂不支持此功能，返回 null 并记录警告日志。
       */
      ubrowserLogin: async () => {
        console.warn('[zbrowser] ubrowserLogin is not supported in MTools')
        return null
      }
    })
