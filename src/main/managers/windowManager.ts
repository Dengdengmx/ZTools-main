 ...
 */
class WindowManager {
  private isPinned: boolean = false // 🚨 核心图钉状态

  public setWindowPinned(pinned: boolean): void {
    this.isPinned = pinned
    console.log('[Window] 窗口常驻状态切换为:', pinned)
  }
  ...
  private moveWindowToCursor(): void {
    ...
    if (savedPosition) {
      ...
      // 🚨 获取当前窗口的真实宽度（可能是 800，也可能是 1100），以此来计算居中
      const currentWidth = this.mainWindow.getSize()[0] 
      x = displayX + Math.floor((width - currentWidth) / 2)
      y = displayY + Math.floor((height - WINDOW_DEFAULT_HEIGHT) / 2)
    }
