...
export function getDownloadUrl(isDev, version) {
  if (isDev) {
    return 'https://github.com/MToolsCenter/MTools/releases/download/dev'
  } else {
    return `https://github.com/MToolsCenter/MTools/releases/download/v${version}`
  }
}
...
export function generateDownloadLinksMarkdown(downloadUrl, version) {
  ...
  links += `- 安装版：[MTools-${version}-win-x64-setup.exe](${downloadUrl}/MTools-${version}-win-x64-setup.exe)\n`
  links += `- 便携版：[MTools-${version}-win-x64.zip](${downloadUrl}/MTools-${version}-win-x64.zip)\n\n`

  links += '#### macOS 11+ (Apple Silicon)：\n\n'
  links += `- DMG：[MTools-${version}-mac-arm64.dmg](${downloadUrl}/MTools-${version}-mac-arm64.dmg)\n`
  links += `- ZIP：[MTools-${version}-mac-arm64.zip](${downloadUrl}/MTools-${version}-mac-arm64.zip)\n\n`

  links += '#### macOS 10.13+ (Intel)：\n\n'
  links += `- DMG：[MTools-${version}-mac-x64.dmg](${downloadUrl}/MTools-${version}-mac-x64.dmg)\n`
  links += `- ZIP：[MTools-${version}-mac-x64.zip](${downloadUrl}/MTools-${version}-mac-x64.zip)\n`
