...
OUT_DIR="/data/yc/mx/8ilq_Gn_helix_design/task_c6a6/design_output"
...
for INPUT_PDB in /data/yc/mx/8ilq_Gn_helix_design/task_c6a6/*.pdb; do
    if [ -f "$INPUT_PDB" ]; then
        ...
        python $RFD_SCRIPT \
            ...
            contigmap.contigs="[A21-335/25-25/A354-452]" \
...
for PDB_FILE in $OUT_DIR/rfd_out/*.pdb; do
    if [ -f "$PDB_FILE" ]; then
        ...
        FIXED_TPL="/data/yc/mx/8ilq_Gn_helix_design/task_c6a6/c6a6_${ORIG_NAME}_fixed_template.jsonl"
        if [ ! -f "$FIXED_TPL" ]; then FIXED_TPL="/data/yc/mx/8ilq_Gn_helix_design/task_c6a6/c6a6_Target_fixed_template.jsonl"; fi
        if [ -f "$FIXED_TPL" ]; then
...
cd /data/yc/mx/8ilq_Gn_helix_design/task_c6a6/
